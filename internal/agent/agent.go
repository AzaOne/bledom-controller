...
func NewAgent(cfg *config.Config) (*Agent, error) {
	...
	// Initialize MQTT Client (if enabled in config)
	a.mqttClient = mqtt.NewClient(cfg, a.bleController, a.luaEngine)

	// Use the method handlePatternStatusChange instead of a local closure
...
// handlePatternStatusChange is called whenever a Lua pattern starts or stops.
// It updates internal state and broadcasts to both WebSockets and MQTT.
	...
	bleStatusCallback := func(connected bool, rssi int16) {
		...
		if !wasConnected && connected {
			...
			if patternToResume != "" {
				...
				// Pass the class method callback
